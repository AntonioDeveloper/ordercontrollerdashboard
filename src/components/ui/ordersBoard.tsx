...
export default function OrdersBoard () {
  ...
  useEffect(() => {
    async function fetchData() {
      // You can await here
      const response = await fetch("http://127.0.0.1:3001/api/orders").then(res => res.json()).then(data => data)
  ...
  useEffect(() => {
    ...
    async function updateOrderStatus () {
      try{
        const response = await fetch(`http://127.0.0.1:3001/api/orders/update/${activeOrderId}`, {
          method: "PUT",
          headers: {
